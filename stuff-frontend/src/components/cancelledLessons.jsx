...
import {getFormatedTime, getFullName} from "../utils/formating.js";
...
export default function CancelledLessons({fetchLessons, byPerson}) {
  ...
  return (
    Array.isArray(lessons) && lessons.length > 0 ? (
      <Box component="div">
        ...
        >
          <Typography variant="h6" sx={{paddingX: 1, paddingTop: 2}}>Отмена занятий</Typography>
          <IconButton
                ...
                {lessons.map(lesson => (
                  <Stack direction="row" key={lesson.id}>
                    <ListItem key={lesson.id}>
                      ...
                      <ListItemText
                        primary={byPerson === STUFF.TEACHER ?
                          (getFullName(lesson.lesson.student)) : (getFullName(lesson.lesson.teacher))}
                        secondary={`Отменено ${lesson.date} ${getFormatedTime(lesson.lesson.timestamp)}`}
                      />
                    </ListItem>
                    <IconButton
